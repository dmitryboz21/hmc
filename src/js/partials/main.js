$(document).ready(function () {
	...
	var chartWrap = $('#numpurchases-chart-wrap'),
		...
		chartWrapH = chartWrap.height(),
		chartWrapW = chartWrap.width();

	function resize_my_canvas() {
		chartWrapH = chartWrap.height();
		chartWrapW = chartWrap.width();
		...
		this.draw = function (data) {
			...
			for (var i = 0; i < numberOfParts; i++) { //numberOfParts
				...
				/*
									...
									console.log(radToAngle(0));
									console.log(radToAngle(df));
									var newPoint=getPoint(this.x, this.y, this.radius+0.5*this.lineWidth,df);
									canvas.moveTo(newPoint[0], newPoint[1]); // Начало линии

									var newPoint=getPoint(this.x, this.y, this.radius+0.5*this.lineWidth,df + (Math.PI * 2) * (parts[i] / 100));
									canvas.lineTo(newPoint[0], newPoint[1]); // Конец линии
